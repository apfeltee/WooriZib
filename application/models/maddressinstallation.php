 ...
 * @author	Dejung Kang
 */
class Maddressinstallation extends CI_Model {
	...
	function get_sido(){
		...
		$this->db->select("sido");
		if($this->type=="front"){
			$this->db->where('`id` IN (SELECT `address_id` FROM `installations` WHERE is_activated = 1)', NULL, FALSE);
		}
	...
	/**
	 * 지하철역의 매물 수를 반환해 주는데 installations에서 is_activated 값이 1일 경우에만 계산해야 해서 수정
	 * 다시 조인을 해야하기 때문에 어쩔 수가 없다.
	 * 
	 */
	function get_subway($search){
		$display_query = (!$this->config->COMPLETE_DISPLAY) ? "and installations.is_finished='0'" : "";

		$this->db->select("subway.*");
		$this->db->select(" (select count(*) from installation_subway join installations on installations.id=installation_subway.installation_id where installations.is_activated='1' and installations.is_valid='1' ".$display_query." and installation_subway.subway_id=subway.id) as cnt");
	...
	function get_local($type){
		...
		$this->db->select("installations.address_id");
		...
		if(!$this->config->COMPLETE_DISPLAY){
			$this->db->where("installations.is_finished","0");
		}
...
/* End of file maddressinstallation.php */
/* Location: ./application/models/maddressinstallation.php */