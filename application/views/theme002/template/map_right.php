<?php 
	...
	<div class="relists relist list_<?php echo $val["id"]?>" data-lat="<?php echo $val["lat"]?>" data-lng="<?php echo $val["lng"]?>" style="border-bottom:1px solid #efefef;">
		...
		<div class='info_desc'>
			<div>
				<div style='height:28px;overflow:hidden;font-weight:700;border-bottom:1px solid #cacaca;'>
				<?php if($val["category_opened"]=="N" && !$this->session->userdata("id")){?>
					<a href="#" class="leanModal" lean-id="#signup" title="<?php echo $val["title"];?>"><?php echo cut($val["title"],35)?></a>
				<?php } else {?>
					<?php if($this->session->userdata("permit_area")){
							$permit_area = @explode(",",$this->session->userdata("permit_area"));
							if(in_array($val["address_id"],$permit_area)){?>
								<a href="#" class="view_product" data-id="<?php echo $val["id"];?>" data-toggle="modal" data-target="#view_dialog" title="<?php echo $val["title"];?>"><?php echo cut($val["title"],35)?></a>
							<?php } else {?>
								<a href="#" class="leanModal" lean-id="#permit-area" title="<?php echo $val["title"];?>"><?php echo cut($val["title"],35)?></a>			
							<?php }?>
					<?php } else {?>
						<a href="#" class="view_product" data-id="<?php echo $val["id"];?>" data-toggle="modal" data-target="#view_dialog" title="<?php echo $val["title"];?>"><?php echo cut($val["title"],35)?></a>
					<?php }?>
				...
				<div class='address'>
					...
					<i class='fa fa-building-o'></i> <?php echo $val["current_floor"];?>층 
					<?php } ?>
