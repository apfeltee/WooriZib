...
function get_comment(){
	$.getJSON("/portfoliocomment/get_json/<?php echo $query->id;?>/"+Math.round(new Date().getTime()),function(data){
			var str = "";
			$.each(data, function(key, val) {
				...
				if(val["member_id"]=="0") {
					...
					str += "<a href='#editModal' role=\"button\" data-toggle=\"modal\" onclick=\"open_edit('"+val["id"]+"',"+val["step_id"]+",'"+val["portfolio_id"]+"')\"><i class='fa fa-pencil-square-o'></i> <?php echo lang("site.modify");?></a>";
				} else {
					if(val["member_id"]=="<?php echo $this->session->userdata("id");?>") {
						str+=" <button type='button' class='btn btn-small' style='padding:0px 5px 0px 2px;'><?php echo lang("site.delete");?></button> <button type='button' class='btn btn-small' style='padding:0px 5px 0px 2px;'>수정</button>";
					}
				}
				if(val["delete"]=="Y"){
...
function open_edit(id,step_id,portfolio_id){
	$("#edit_id").val(id);
	$("#edit_step_id").val(step_id);
	$.getJSON("/portfoliocomment/get_one_json/"+id+"/"+step_id+"/"+portfolio_id+"/"+Math.round(new Date().getTime()),function(data){
		$.each(data, function(key, val) {
              ...
              <div class="row">
                ...
                <div class="col-lg-9 news-item">
                  ...
                  <div class="news-item-img">
							<?php if($query->thumb_name!=""){
								echo "<img src='/uploads/portfolios/".$query->thumb_name."' class='img-responsive'>";
							}?>
                  </div>
                  ...
                  <div class="post-comment padding-top-40">
                    <h3>댓글 남기기</h3>
					  <?php echo form_open("portfoliocomment/add_action","id='add_form'");?>
					  <!-- 회원일 경우에는 비밀번호를 입력하지 않는다 -->
					  <input type="hidden" name="portfolio_id" value="<?php echo $query->id;?>">
					  <input type="hidden" name="member_id" value="<?php echo $this->session->userdata("id");?>">
					  <input type="hidden" name="temp" value="1313">
                    <form role="form">
                      <div class="form-group">
                        <label><?php echo lang("site.name");?></label>
						<input type="text" id="comment_add_name" name="name" autocomplete="off" class="form-control">
                      </div>
                ...
                <div class="col-lg-3 news-sidebar">
                  ...
                  <ul class="nav sidebar-categories margin-bottom-40">
						<li> <a href="/portfolio/index"><?php echo lang("site.all");?></a></li>
						<?php foreach($portfoliocategory as $val){?>
						 <li <?php if($query->category==$val->id){?>class="active"<?php }?>><a href="/portfolio/index/<?php echo $val->id;?>/" style="cursor:pointer;"><?php echo $val->name;?><?php if($val->opened=="N") {echo " <i class='fa fa-user'></i>";}?></a></li>
						<?php }?>
                  </ul>
                  <!-- CATEGORIES END -->
...
<!-- 댓글에 답글 달기 -->
<?php echo form_open("portfoliocomment/add_reply_action","id='reply_form' class='form-horizontal'");?>
    ...
    <div class="modal-content">
	  ...
	  <div class="modal-body">
		...
		<div class="control-group">
			...
			<div class="controls">
				<input type="text" name="name" id="reply_name" autocomplete="off">
			</div>
		</div>
...
<!-- 수정하기 -->
<?php echo form_open("portfoliocomment/edit_action","id='edit_form' class='form-horizontal'");?>
    ...
    <div class="modal-content">
	  ...
	  <div class="modal-body">
		...
		<div class="control-group">
			<label class="control-label"><?php echo lang("site.name");?></label>
			<div class="controls">
				<input type="text" id="comment_edit_name" name="name" autocomplete="off">
			</div>
		</div>
		<div class="control-group" <?php if($this->session->userdata("id")!="") {echo "style='display:none;'";}?>>
			<label class="control-label"><?php echo lang("site.pw");?></label>
...
<!-- 삭제하기 -->
<?php echo form_open("portfoliocomment/delete_action","id='delete_form' class='form-horizontal'");?>
