...
$(document).ready(function(){
	...
	<?php 
	if($store_data){
		foreach($store_data as $store_key=>$store){
	?>
			var index = <?php echo $store_key?>;
			var title = '<?php echo $store->title;?>';
			var lat = <?php echo $store->latitude?>;
			var lng = <?php echo $store->longitude?>;
			markers[index] = new daum.maps.Marker({
				position: new daum.maps.LatLng(lat, lng),
				map: map_detail,
				title: title
    ...
    <div class="row">
        <div class="col-md-9 col-xs-12">
			<div class="property_content shadow-border">
					<h3 style="font-weight:bold;margin-top:20px;">[<?php echo $query->id?>] <?php echo $query->title?>
					<?php if($query->is_finished=="1"){?>
						<span style="font-size:15px;border-radius:5px;color:white;margin-left:3px;padding:0px 5px 2px 3px;line-height:1em;background-color:red;"><?php echo ($query->type=="installation") ? "분양" : "계약"?>완료</span>
					<?php } else {?>
						<?php if($query->is_defer=="1"){?>
							<span style="font-size:15px;border-radius:5px;color:white;margin-left:3px;padding:0px 5px 2px 3px;line-height:1em;background-color:red;"><?php echo ($query->type=="installation") ? "분양" : "계약"?>보류</span>
						<?php }?>
					<?php }?>
					</h3>
		...
		<div class="col-md-3 col-xs-12">
				...
				<div class="member shadow-border question">
					...
					<div class="question_body">
						<?php if($config->GONGSIL_FLAG){?>
							<?php echo multi_view($query->gongsil_contact,"gongsil");?><br>
							...
							<div class="row">
								...
								<div class="col-md-12 margin-top-10 view_phone_area <?php if($config->CALL_HIDDEN){?>hidden<?php }?>">
									...
									if($member->type!="admin"){
										...
										<?php } else {?>
										<div class="i_title"><?php echo $member->address?> <?php echo $member->address_detail?></div>
										<div class="i_title">중개등록번호 : <?php echo $member->re_num?></div>
										<div class="i_title">대표전화 : <?php echo $member->tel?></div>
										<div class="i_title">대표자 : <?php echo $member->biz_ceo?></div>
									<?php 
										}
			...
			<div class="tab-content">
			  ...
			  <?php
				$recent_title = ($query->danzi_id) ? "같은 단지" : "인근 "; 
			  ?>
			  <h4><?php echo $recent_title?> <?php echo lang("product");?> <i class="fa fa-question-circle help" data-toggle="tooltip" title="<?php echo lang("product");?> 중 가까운 순으로 보여집니다."></i></h4>
			  <ul class="list_widget">
