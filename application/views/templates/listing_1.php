...
foreach($product as $val){
	...
	title="<?php echo element("title",$val)?>">
		...
		<td>
			...
			<?php if( element("recommand",$val)=="1")  { ?><span class="label label-sm label-success"><?php echo lang("site.recommand");?></span><?php }?>
				...
				if(element("part",$val) && $val["part"]=="Y"){
					if($config->AREA_SORTING){ //실면적 우선
						if(element("real_area",$val)!="0"){
							echo " ". area_list(element("real_area",$val),"") ;
						} else if(element("law_area",$val)!="0"){
							echo " ". area_list(element("law_area",$val),"") ;
						}
					}
					else{ //계약면적면적 우선
