	...
	<div class="relists relist list_<?php echo $val["id"]?>" data-lat="<?php echo $val["lat"]?>" data-lng="<?php echo $val["lng"]?>">
		...
		<div class='info_desc'>
			<div>
				...
				<?php if($val["category_opened"]=="N" && !$this->session->userdata("id")){?>
					<a href="#" class="leanModal" lean-id="#signup" title="<?php echo $val["title"];?>"><?php echo cut($val["title"],45)?></a>
				<?php } else {?>
					<?php if($this->session->userdata("permit_area")){
							$permit_area = @explode(",",$this->session->userdata("permit_area"));
							if(in_array($val["address_id"],$permit_area)){?>
								<a href="#" class="view_product" data-id="<?php echo $val["id"];?>" data-toggle="modal" data-target="#view_dialog" title="<?php echo $val["title"];?>"><?php echo cut($val["title"],45)?></a>
							<?php } else {?>
								<a href="#" class="leanModal" lean-id="#permit-area" title="<?php echo $val["title"];?>"><?php echo cut($val["title"],45)?></a>			
							<?php }?>
					<?php } else {?>
						<a href="#" class="view_product" data-id="<?php echo $val["id"];?>" data-toggle="modal" data-target="#view_dialog" title="<?php echo $val["title"];?>"><?php echo cut($val["title"],45)?></a>
					<?php }?>
